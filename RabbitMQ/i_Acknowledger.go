...
func (consumer *RabbitMQConsumer) Acknowledge(acknowledge dto_broker.BrokerAcknowledge) (err error) {
	...
	if !found {
		return fmt.Errorf("not found message id %s from consumer %s of queue %s", acknowledge.MessageId, consumer.Name, consumer.Queue.Name)
	}
	...
	case amqp.Delivery:
		...
		case constant_broker.ACKNOWLEDGE_SUCCESS:
			...
			if err != nil {
				return fmt.Errorf("error positive acknowledging message id %s from consumer %s of queue %s: %w", acknowledge.MessageId, consumer.Name, consumer.Queue.Name, err)
			}
		case constant_broker.ACKNOWLEDGE_REQUEUE:
			err = message.Acknowledger.Nack(message.DeliveryTag, false, true)
			if err != nil {
				return fmt.Errorf("error negative acknowledging with requeue message id %s from consumer %s of queue %s: %w", acknowledge.MessageId, consumer.Name, consumer.Queue.Name, err)
			}
		case constant_broker.ACKNOWLEDGE_DEADLETTER:
			err = message.Acknowledger.Nack(message.DeliveryTag, false, false)
			if err != nil {
				return fmt.Errorf("error negative acknowledging with deadletter message id %s from consumer %s of queue %s: %w", acknowledge.MessageId, consumer.Name, consumer.Queue.Name, err)
			}
	...
	default:
		return fmt.Errorf("message id %s have come with untreatable %T format from consumer %s of queue %s", acknowledge.MessageId, mapObject, consumer.Name, consumer.Queue.Name)
	}
